...
function isOnline() {
  // In Electron, navigator.onLine is available in renderer, not main. Here, always try Supabase if initialized.
  return !!supabase;
}
...
// Unified DB: delegates to Supabase or localDb based on connectivity
export const unifiedDb = new Proxy({}, {
  get(target, prop) {
    // Prefer Supabase if online, else fallback to localDb
