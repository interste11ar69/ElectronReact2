...
contextBridge.exposeInMainWorld(
  "electronAPI",
  {
    // Single object literal starts here
    // --- Authentication ---
    login: (credentials) => {
    ...
    updateItem: (itemDataWithId) => {
      // Renderer sends {id, ...otherData}
      console.log(
        '[Preload] Invoking "update-item" with data:',
        itemDataWithId
      );
    ...
    getLowStockItems: (threshold) => {
      console.log(
        '[Preload] Invoking "get-low-stock-items" with threshold:',
        threshold
      );
    ...
    updateCustomer: (customerDataWithId) => {
      // Renderer sends {id, ...otherData}
      console.log(
        '[Preload] Invoking "update-customer" with data:',
        customerDataWithId
      );
    ...
    performStockAdjustment: (details) => {
      console.log(
        '[Preload] Invoking "perform-stock-adjustment" with details:',
        details
      );
    ...
    updateBundle: (bundleId, bundleData) => {
      // Changed from (id, bundleData) to (bundleId, bundleData) to match main.js likely
    ...
    processBundleSale: (args) => {
      // args will be { bundleId, quantitySold }
    ...
    createSalesOrder: (orderData, orderItemsData) => {
      ...
      return ipcRenderer.invoke("create-sales-order", {
        orderData,
        orderItemsData,
      });
    },
    getSalesOrders: (filters) => {
      console.log(
        '[Preload] Invoking "get-sales-orders" with filters:',
        filters
      );
    ...
    getSalesOrderById: (orderId) => {
      console.log(
        '[Preload] Invoking "get-sales-order-by-id" for Order ID:',
        orderId
      );
    ...
    updateSalesOrderStatus: (orderId, newStatus) => {
      console.log(
        `[Preload] Invoking "update-sales-order-status" for Order ID: ${orderId} to Status: ${newStatus}`
      );
      return ipcRenderer.invoke("update-sales-order-status", {
        orderId,
        newStatus,
      });
    },
    ...
    getInventoryTransactionsForItem: (itemId, limit, offset) => {
      console.log(
        `[Preload] Invoking "get-inventory-transactions-for-item". Args:`,
        { itemId, limit, offset }
      );
      // Always package arguments into a single object for the main process
      return ipcRenderer.invoke("get-inventory-transactions-for-item", {
        itemId: itemId,
        limit: limit,
        offset: offset,
      });
    ...
    createStockTransfer: (transferDetails) =>
      ipcRenderer.invoke("create-stock-transfer", transferDetails),
    ...
    getSalesSummary: (period) => {
      console.log(
        '[Preload] Invoking "get-sales-summary" with period:',
        period
      );
    ...
    getTopSellingItems: (period, limit) => {
      console.log(
        '[Preload] Invoking "get-top-selling-items" with period:',
        period,
        "limit:",
        limit
      );
    ...
    getSalesByStatus: (period) => {
      console.log(
        '[Preload] Invoking "get-sales-by-status" with period:',
        period
      );
    ...
    getItemQuantityAtLocation: (itemId, locationId) =>
      ipcRenderer.invoke("get-item-quantity-at-location", itemId, locationId),

    performStockAdjustment: (details) =>
      ipcRenderer.invoke("perform-stock-adjustment", details),
    getDetailedStockReport: (filters) => {
      console.log(
        '[Preload] Invoking "get-detailed-stock-report" with filters:',
        filters
      );
    ...
    getSalesDetailReport: (filters) => {
      console.log(
        '[Preload] Invoking "get-sales-detail-report" with filters:',
        filters
      );
      return ipcRenderer.invoke("get-sales-detail-report", filters);
    },
    // Ensure exportReportData is already here from previous step for ReportsPage
    exportReportData: (params) => {
      console.log(
        '[Preload] Invoking "export-report-data" with params:',
        params
      );
      return ipcRenderer.invoke("export-report-data", params);
    },
